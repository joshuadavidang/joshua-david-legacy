const FULL_NAME: string = 'Joshua David';
const DESCRIPTION: string = 'Sophomore at the School of Computing and Information Systems, Singapore Management University';
const ROLE: string = 'Software Engineer Intern';
const INTERNSHIP_HEADER: string = '👨🏻‍💻 Internship Experience';
const CONTACT_HEADER: string = 'Stay Connected 🔥';
const PROJECT_HEADER: string = 'Projects I\'ve done👇🏻';
const RESUME_HEADER: string = '👇🏻 Full Resume';

export { FULL_NAME, DESCRIPTION, ROLE, INTERNSHIP_HEADER, PROJECT_HEADER, RESUME_HEADER, CONTACT_HEADER };